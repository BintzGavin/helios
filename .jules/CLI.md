...
## [v0.46.23] - Identify Uncovered Config Utils
**Learning:** The fallback action from [0.46.22] for templates files was completed. Searching for uncovered areas in a stable domain requires thoroughly checking tests against source files. `packages/cli/src/utils/config.ts` currently lacks test coverage while being a core piece of the CLI configuration parsing.
**Action:** When falling back to regression testing under the NOTHING TO DO PROTOCOL, target configuration parsing utilities (like config.ts) that are critical for command execution when command, registry, and templates coverage is 100%. Created plan `2026-05-12-CLI-Config-Regression-Tests.md`.
**Learning:** The fallback action from [0.46.23] to write regression tests for `cli/src/utils/config.ts` was an IMPOSSIBLE: DUPLICATION. Tests for this utility file are already fully implemented in `packages/cli/src/utils/__tests__/config.test.ts` and are passing.
...
## [v0.46.29] - Job/Render/Merge Tests Resolution Issue
**Learning:** `vitest` failed in `packages/cli/src/commands/__tests__/` (specifically `job.test.ts`, `render.test.ts`, and `merge.test.ts`) due to missing entry resolution for `@helios-project/infrastructure` and `@helios-project/renderer` during import analysis.
**Action:** Always ensure cross-workspace dependencies are either mocked correctly at the top of test files (e.g. `vi.mock('@helios-project/renderer')`) or built before running workspace-specific tests in isolation. Created plan `2027-06-05-CLI-Job-Render-Merge-Regression-Tests-Missing-Mock.md` to document the necessity of these mocks when running tests independently.
## [v0.46.33] - Identify Uncovered Registry Types
**Learning:** The fallback action from [0.46.31] for the index.ts regression test was completed. Searching for uncovered areas in a stable domain requires thoroughly checking tests against source files. `packages/cli/src/registry/types.ts` currently lacks test coverage (using static types in assertions) while defining the core shape of the registry index.
**Action:** When falling back to regression testing under the NOTHING TO DO PROTOCOL, target TypeScript type files (like types.ts) that enforce structural constraints when command, templates, and utils coverage is 100%. Created plan `2027-06-05-CLI-Registry-Types-Regression-Tests.md`.
