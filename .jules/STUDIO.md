## [0.68.0] - Build Dependencies
**Learning:** `packages/studio` build fails with type errors if `packages/core` is not built first, because it depends on the types from core. Also, `packages/studio` lacks a `lint` script.
**Action:** When working on Studio, ensure `packages/core` is built (`npm run build -w packages/core`) before verifying Studio builds. Add `lint` script to `packages/studio/package.json` in a future task.
