...
2.  **DOM Strategy**:
    ...
    *   **Caption Burning**: Supports burning subtitles (.srt) into the video using FFmpeg's `subtitles` filter (requires re-encoding).
...
- `subtitles`: Path to a subtitle file (.srt) to burn into the video.
