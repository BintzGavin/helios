...
This indicates that returning the V8 promise chain natively is significantly faster (~34% faster for this micro-operation), as it removes an intermediate async suspension point and reduces event loop overhead. A similar optimization was successfully implemented in `DomStrategy.ts` (`PERF-432`) which also avoided `async/await` overhead in the hot loop.
