...
# PERF-1006: Isolate DOM and Canvas chunk loops in the single-worker `hasProcessFn` path

## Focus Area
The single-worker fast chunk rendering loop inside `CaptureLoop.ts` when `hasProcessFn` is true.
...
**What to change**:
In the single-worker path, find the chunk loop inside the `if (hasProcessFn)` block (around lines 270-320):
```typescript
            let i = 1;
