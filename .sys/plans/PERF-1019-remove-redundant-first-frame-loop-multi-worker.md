...
# PERF-1019: Remove redundant first-frame write loop in multi-worker path
...
## Canvas Smoke Test
Run `npm run test -w packages/renderer verify-canvas-strategy.ts` to ensure the Canvas strategy path works.

## Correctness Check
Run `npm run test -w packages/renderer verify-dom-strategy-capture.ts` to ensure the DOM path is still functioning correctly. Run `npm run test -w packages/renderer` to ensure nothing is broken.
