...
*Note: In previous experiments (like `PERF-296` and `PERF-333`), inlining inline object literals in `SeekTimeDriver` regressions was because Playwright CDP serializes things asynchronously and mutating a single object in a loop caused race conditions. Here, `targetElementHandle.screenshot()` is awaited synchronously per frame, so there are no async serialization overlap issues.*
...
## Prior Art
- PERF-391: Preallocated singleFrameEvaluateParams in SeekTimeDriver.
- PERF-392: Preallocated multiFramePromises array.
- PERF-393: Preallocated evaluateParams object literals.
