...
# PERF-967: Cache Decoded Base64 Buffers for Unchanged Frames in Single-Worker Loop (!hasProcessFn)
...
### Step 1: Optimize the inner chunked loop and initial frame captures
**File**: `packages/renderer/src/core/CaptureLoop.ts`
**What to change**:
In the `else` block corresponding to `!hasProcessFn`, find the `if (isDomStrategy)` branch.
Update the inner `for` chunk loop:
```javascript
<<<<<<< SEARCH
                  for (; i < chunkEnd; i++) {
...
Update the `totalFrames > 1` final iteration:
```javascript
<<<<<<< SEARCH
...
## Correctness Check
Run `verify-dom-media-preload.ts` or visually verify the DOM output mp4 to ensure frames aren't dropped or duplicated incorrectly.
