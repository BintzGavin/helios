---
id: PERF-846
slug: hoist-aborted-checks-single-worker
status: unclaimed
claimed_by: ""
created: 2024-06-25
completed: ""
...
# PERF-846: Hoist Redundant `aborted` Branch Checks in Single-Worker CaptureLoop Fast Paths

## Focus Area
`packages/renderer/src/core/CaptureLoop.ts` fast paths (single worker loop).

## Background Research
In PERF-838 and PERF-839, we successfully hoisted the redundant `aborted` boolean checks in the multi-worker capture loops, yielding a significant improvement in V8 hot loop optimization.
...
You must systematically trace through all `for (let i = 1; i < totalFrames; i++)` loops inside the single-worker block (`if (hasProcessFn) { ... } else { ... }`) and remove the redundant inner `if (aborted) break;` statements. Leave only the initial `if (aborted) break;` at the top of the `for` loop.

There are exactly 8 such inner checks that need to be removed:
- `if (isDomStrategy)` string loop
- `else` string loop
- `if (isDomStrategy)` buffer loop
- `else` buffer loop
- And the same 4 again for the `!hasProcessFn` branch.
