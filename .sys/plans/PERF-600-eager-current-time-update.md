...
# PERF-600: Eager Update of currentTime to Eliminate .then in CdpTimeDriver
...
Because workers strictly `await` the `setTime` promise before processing the next frame, `runSetTime` is never called concurrently on the same driver instance. Therefore, we can safely *eagerly* update `this.currentTime = timeInSeconds` right before returning the CDP Promise, completely eliminating the need for the `.then()` chain and avoiding modifications to the CDP event handler.
...
Find this block (around lines 228-232):
```typescript
...
**Risk**: If `runSetTime` were ever called concurrently before the promise resolved, the eager update would calculate an incorrect `delta` for the second call. However, the `CaptureLoop.ts` orchestrator strictly serializes frame processing per worker, so concurrency per `TimeDriver` instance is impossible.
