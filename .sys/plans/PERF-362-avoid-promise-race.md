...
# PERF-362: Avoid Promise.race allocation in SeekTimeDriver injected script
...
```
This forces the browser to allocate a new `Promise` (for the timeout) and a new array for `Promise.race`, as well as a wrapper Promise, on every frame where stability checks are present.

A simpler, more performant way to implement a timeout without allocating a `timeoutPromise` and `Promise.race` array on every execution is to use a manual wrapper promise that races the `allReady` promise and the timeout directly:
...
## Results Summary
- **Best render time**: 48.513s (vs baseline 48.533s)
- **Improvement**: 0%
- **Kept experiments**: []
- **Discarded experiments**: [PERF-362]
