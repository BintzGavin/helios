...
Converting this to a recursive `.then()` chain is a known pattern for optimizing hot asynchronous loops in Node.js. Note that previous experiments like PERF-472 failed because the `timeDriver.setTime` and `strategy.capture` operations are inherently asynchronous and removing the async wrappers without keeping the execution logic asynchronous breaks backpressure or limits callstack if not careful. By simply returning the promises we bypass the `async` function overhead but keep the flow asynchronous.
