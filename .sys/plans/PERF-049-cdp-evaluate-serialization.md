...
## Baseline
- **Current estimated render time**: ~32s
...
**What to change**:
In `SeekTimeDriver.ts`, locate the `Runtime.evaluate` call in `setTime` and change `returnByValue: true` to `returnByValue: false`.
...
**File**: `packages/renderer/src/drivers/SeekTimeDriver.ts`
**What to change**:
In the `window.__helios_seek` script, locate the `console.warn` related to GSAP timeline availability inside the `else if (!gsapTimelineSeeked)` block and remove it or comment it out.
