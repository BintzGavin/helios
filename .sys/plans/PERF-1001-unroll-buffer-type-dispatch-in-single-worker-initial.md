...
# PERF-1001: Unroll buffer type dispatch in single-worker initial frame processing
...
**What to change**:
In the `if (hasProcessFn) { if (isDomStrategy) {` block for the first frame (around line 226), cast `domLastFrameBuffer` to `Buffer`:
```typescript
...
**File**: `packages/renderer/src/core/CaptureLoop.ts`
**What to change**:
In the `} else {` block (for `!hasProcessFn`), inside `if (1 < totalFrames) { ... }`, find the merged writing logic (around line 407):
```typescript
            let buf: any;
            if (isDomStrategy) {
...
```
Replace it by moving the write logic inside the `if (isDomStrategy)` branches and applying explicit casts:
```typescript
            if (isDomStrategy) {
              const data = (rawResult as any).screenshotData;
              if (data) {
