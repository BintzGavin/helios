...
#### 2. File Inventory
- **Create**: `packages/infrastructure/tests/e2e/deterministic-seeking.test.ts` - New E2E test suite.
- **Modify**: `packages/infrastructure/package.json` - Add test script or dependency if required.
- **Read-Only**: `packages/infrastructure/src/adapters/aws-adapter.ts`, `packages/infrastructure/src/adapters/cloudrun-adapter.ts` - Read for execution context.
