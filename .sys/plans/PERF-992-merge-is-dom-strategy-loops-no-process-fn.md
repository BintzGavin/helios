...
# PERF-992: Merge duplicated chunk writer loops in single-worker no-process-fn path

## Focus Area
The single-worker `!hasProcessFn` path in `packages/renderer/src/core/CaptureLoop.ts` (around line 410) has completely duplicated while loops for `if (isDomStrategy)` and its `else` branch (Canvas strategy).
...
**What to change**:
Instead of `if (isDomStrategy) { while (...) { ... } } else { while (...) { ... } }` in the `!hasProcessFn` branch (around line 410), merge them into a single loop:
