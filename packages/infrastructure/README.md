...
- **JobManager**: The central orchestrator for distributed job lifecycles. It provides high-level APIs to `submitJob`, `pauseJob`, `resumeJob`, `cancelJob`, or `deleteJob` active rendering jobs. It integrates transparently with `ArtifactStorage` to handle automatic pre-execution asset uploads (`uploadAssetBundle`, `uploadJobSpec`) and post-execution cleanup (`deleteAssetBundle`, `deleteJobSpec`), aggregates execution metrics (task durations, chunk-level logs), and manages distributed state persistence via pluggable `JobRepository` implementations (such as `FileJobRepository`). It also provides telemetry hooks for realtime progress and log streaming.
...
Cloud adapters provide the critical abstraction layer that translates standardized, internal worker jobs into provider-specific cloud compute API invocations (`executeChunk`, `mergeChunks`), enabling truly infrastructure-agnostic distributed rendering.
