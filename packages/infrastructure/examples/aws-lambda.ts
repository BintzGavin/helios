...
async function main() {
  ...
  const jobDefUrl = 'https://example.com/jobs/mock-job.json'; // Where workers fetch the job spec
  ...
  try {
    const submittedId = await manager.submitJob(jobSpec, {
      concurrency: 2, // AWS Lambda scales automatically, but we control parallel invocations here
      jobDir: process.cwd()
    });
