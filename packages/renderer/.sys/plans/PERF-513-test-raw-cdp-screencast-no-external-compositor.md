...
This plan aims to modify `BrowserPool.ts` to remove the conflicting arguments, and update `DomStrategy.ts` to use `Page.startScreencast` instead of `HeadlessExperimental.beginFrame` for the capture loop, waiting for screencast frames and falling back to the last frame on timeout if there's no damage.
...
## Baseline
- **Current estimated render time**: ~19.93s
...
- In `finish()`, stop the screencast with `Page.stopScreencast` before setting `this.cdpSession` to null.
