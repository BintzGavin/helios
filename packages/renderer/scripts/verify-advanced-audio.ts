...
const runTest = () => {
  ...
  const configVolume = domStrategy.getFFmpegArgs(optionsVolume, outputPath);
  const argsVolume = configVolume.args;
  ...
  const configDelay = domStrategy.getFFmpegArgs(optionsDelay, outputPath);
  const argsDelay = configDelay.args;
  ...
  const configSFDelay = domStrategy.getFFmpegArgs(optionsStartFrameDelay, outputPath);
  const argsSFDelay = configSFDelay.args;
  ...
  const configSFSeek = domStrategy.getFFmpegArgs(optionsSFSeek, outputPath);
  const argsSFSeek = configSFSeek.args;
  ...
  const configMix = domStrategy.getFFmpegArgs(optionsMix, outputPath);
  const argsMix = configMix.args;
