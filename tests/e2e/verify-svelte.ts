...
async function main() {
  console.log('Starting Svelte verification render...');
  ...
  const compositionPath = path.resolve(
    process.cwd(),
    'output/example-build/examples/svelte-canvas-animation/composition.html'
  );
  const compositionUrl = `file://${compositionPath}`;

  const outputPath = path.resolve(process.cwd(), 'output/svelte-render-verified.mp4');
