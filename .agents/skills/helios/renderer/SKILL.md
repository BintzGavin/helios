...
interface RendererOptions {
  ...
  subtitles?: string;            // Path to SRT file to burn into video (requires libx264)
...
interface AudioTrackConfig {
  ...
  fadeInDuration?: number; // Duration in seconds to fade in
  fadeOutDuration?: number; // Duration in seconds to fade out
  loop?: boolean;    // Whether to loop the audio track
  duration?: number; // Duration of the source audio in seconds (if known)
}
```
