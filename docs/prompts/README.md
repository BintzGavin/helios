...
**Infrastructure**: All scheduling is handled through the [Jules website dashboard](https://jules.ai) (as of now scheduled tasks are supported on all plans afaik). No custom infrastructure needed. The Ultra plan allows up to 300 tasks per day, which is more than enough for multiple cycles. Tasks are scheduled directly through their web interface—morning planning tasks, afternoon execution tasks, and end-of-day scribe tasks. 
